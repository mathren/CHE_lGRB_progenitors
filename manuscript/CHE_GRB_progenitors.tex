...
emerging \citep[e.g.,][]{sukhbold:16, limongi:18, boccioli:24,
...
progenitors which collapse with a ONeMg-rich core (however, these too
...
required to follow weak inreactions in detail (neglecting
...
$T_{\rm eff}$ excursions feedback non-linearily on the core structure
in structurally significant ways, motivating our choice to exclude
models showing this numerical behavior from our grid. Overall, our
...
models (green checkmarks in \Figref{fig:grid_success}).
...
thin black dot-dashed line is the same model computed with a 22-isotope
...
evolve from the bottom right towards the top left. Because of the
...
enriched in $^{4}\mathrm{He}$ (and $^{14}\mathrm{N}$ at the expenses
...
We note that the majority of the models failing (red crosses in
...
creating a profile consistent with the thermodynamical conditions in
the evolving cores.

Comparing the thick solid orange line to the black dot-dashed line, we
see the difference between these two $Y_{e}(m)$ profiles is larger
than the difference between models of varying mass and initial
rotations with large networks: the systematic error from using a small
nuclear reaction network biases conclusions on the connection between
...
radial distribution of mass $r(m)$. Based on the comparison between
the two $40\,M_{\odot}$, $\omega=0.6\,\omega_{\rm crit}$ models, the
collapse dynamics expected may be completely different based on the
weak interactions treatment.
...
whether a disk will form at collapse \cite[e.g.,][]{macfadyen:94,
...
\begin{figure}[tbp]
  ...
  \caption{Compactness as a function initial mass and rotation rate
...
Overall, slower rotation (darker colors) tend leads to higher
compactness, while increasing rotational support decreases slightly
the core density and decreases $\xi_{1.75}$, but the trend is not
...
much angular momentum to directly collapse in a BH.
