...
128-isotope (orange) and 22-isotope (black) nuclear reaction network
shows how the surface $L$ and $T_{\rm eff}$ are relatively insensitive
to
...
the evolving cores. We also show with a thin black dot-dashed line the
$40\,M_{\odot}$, $\omega_{\rm ZAMS}=0.6\,\omega_{\rm crit}$ model from
\cite{renzo:24} computed with a small 22-isotope network and an
otherwise identical setup, for comparison with the thick solid orange
line corresponding to the same initial conditions in our grid using a
128-isotope network. The difference between these two $Y_{e}$ profiles
is larger than the difference between models of varying mass and
initial rotations: the systematic error from using a small nuclear
reaction network biases conclusions on the connection between
