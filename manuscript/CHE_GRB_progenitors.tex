...
Since the connection between massive-star populations and supernovae
...
multi-physics (and often multi-million-CPUh) simulations
...
Recent progress supernova (SN) theory has produced unprecedented
...
The use of small nuclear reaction networks is an acceptable trade-off
...
simulations \citep[e.g.,][]{mueller:16,laplace:25}. However, it is not sufficient
if one wants to investigate the driving of the explosions, the
pre-explosions and explosive neutrino signals, the nuclear yields, and
the resulting compact object properties (whether considering
non-rotating -- \citealt{farmer:16} -- or rotating progenitors --
\citealt{renzo:24}). The reason is that the effective Chandrasekhar
...
to a level significant for the collapse and explosion dynamics
...
far, they do not allow the community to explore the sensitivity of the
results to \emph{physical} and \emph{numerical} choices in the
evolution of the progenitors. Approximations introduced at both these
conceptual levels have been shown to influence the core structure and
...
fast-rotating progenitors exist: to the best of our knowledge only a
set of \textsc{KEPLER} models from \cite{woosley:06} using the
adaptive nuclear reaction network from \cite{heger:00}, and the
\textsc{MESA} models by \cite{aguilera-dena:18, aguilera-dena:20}
...
of evolved massive stars with the large nuclear-reaction networks
necessary to follow in details the weak reactions (neglecting
sparsity, the size of the matrix to solve scales with $\sim{}N^{2}$),
