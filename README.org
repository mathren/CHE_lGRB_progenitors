...
*⚠️ WARNING*: the compressed dataset is ~ 8GB, and becomes ~15GB once
...
*⚠️ WARNING*: The zenodo repository is not publicly accessible yet.
Before running the =build.sh= script (or, specifically, the =data_prep.sh=
script) you need to:
...
*⚠️ WARNING*: the exact code used is available on zenodo in
