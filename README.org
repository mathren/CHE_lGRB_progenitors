...
don't want to stare at the build prompts, run =build.sh --yes=.

If you want to run the full build except one of the steps you can pas
...
- [X] how do address MESA =r24.03.1= bug in nuclear reaction networks (@debraheem?)
...
*** Jared's comments
- [ ] check caption of fig. 5 for comment
- [ ] light pedantic point, but some of these flat steps are radiative
  (i.e. ever-so-slightly positive gradient), structured by prior
  episodes of convection with longer timescales for thermal
  adjustment. They aren't always currently undergoing efficient
  convection. This is part of what gets tricky when your burning and
  convection layers are changing
- [ ] "when the outer layers of these envelope-less stars can be
  dynamically coupled to the core" Are you implying that the outer
  layers are truly dynamically coupled, or just that the timestep
  shrinks long enough to start trying to resolve hydrodynamical
  phenomena, and then hydro things wiggle the model around via both
  physics and numerics? I think what we're dealing with is the latter;
  eventually hydro timescales are still longer than core timescales,
  and the coupling comes in via our improper engineering assumptions
  about transport (
